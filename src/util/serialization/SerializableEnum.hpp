...
#define SERIALIZABLE_ENUM(NAME, ...)                                                                          \
    struct __##NAME {                                                                                         \
      public:                                                                                                 \
        ...
        {                                                                                                     \
            const std::unordered_map<Enum, std::string> map = {FOR_EACH(_ENUM_TO_STRING_ENTRY, __VA_ARGS__)}; \
            return stream << map.at(e);                                                                       \
        }                                                                                                     \
        friend inline std::istream &operator>>(std::istream &stream, Enum &e)                                 \
        {                                                                                                     \
            const std::unordered_map<std::string, Enum> map = {FOR_EACH(_STRING_TO_ENUM_ENTRY, __VA_ARGS__)}; \
            std::string buffer;                                                                               \
                                                                                                              \
            stream >> buffer;                                                                                 \
            e = map.at(buffer);                                                                               \
